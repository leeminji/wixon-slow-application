...
class Member extends SL_Controller {
	...
	public function update(){
		...
		$option = $data;
		$option['title'] = "[회원이름] - 회원관리";

		$this -> _view("/setting/member/write_v",$option);
	}
	...
	public function json_level(){
	    ...
	    if( $_POST ){
			...
			$data['level'] = $this -> level_m -> get_level_list();
			$data['title'] = "회원권환관리";
	        $this -> _layer_view('setting/member/level_v', $data);
	    }
	}
	...
	public function lists(){
		...
		//회원레벨리스트
		$option['level'] = $this -> level_m -> level_list();
		...
		$option['total'] = $config['total_rows'];

		//페이지네이션 라이브러리 로딩 추가
		$this -> load -> library('pagination');
		
		//쿼리스트링
		$option['q'] = "?stx=".$this -> stx."&sfl=".$this -> sfl."&sst=".$this->sst."&sod=".$this->sod;
		$option['mb_level'] = $this -> mb_level ;
		
		//페이지네이션 설정
		$config['base_url'] = '/setting/member/'.$option['q'];
		...
		$option['page'] = floor($per_page/$this->col_per_page)+1;

		$limit = $config['per_page'];
		$option['list'] = $this -> member_m -> member_list( '', $start, $limit, $this -> stx, $this -> sfl, $this -> sst, $this->sod , $this -> mb_level );
		
		for( $i = 0; $i < count($option['list']); $i++){
			//번호매기기 ( 총갯수 - (페이지번호-1 * 페이지당 데이터갯수)- 순차)
			$option['list'][$i]->num = $config['total_rows'] - (($option['page']-1) * $this->col_per_page )-$i;
			
			//검색시 마크설정
			if( $this -> stx && $this -> sfl == 'mb_id' ){
			$option['list'][$i]->mb_id = str_replace( $this -> stx, "<mark>".$this -> stx."</mark>", $option['list'][$i]->mb_id );
			}
			$option['list'][$i]->link = "/setting/member/update/{$option['list'][$i]->mb_idx}/{$this->midx}{$option['q']}";
	...
	public function state(){
		if( $_POST ){
			...
			$update_href = "/setting/member/update/{$mb_idx}/{$this->midx}";
			if( $result ){
			    if( $mb_state == 0 ){
			        alert("탈퇴처리되었습니다.", $update_href);
			    }else{
			        alert("재가입 처리되었습니다.", $update_href);
			    }
			}else{
			    alert("실패하였습니다.", $update_href);
			}
		}
	}
