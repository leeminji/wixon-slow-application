...
class Level_m extends CI_Model{
	...
	//회원레벨
	function level_list(){
		$sql = "SELECT count(mb_level) as cnt , ml_name , ml_idx FROM {$this->table} as level RIGHT JOIN {$this->member_table} as member on level.ml_idx = member.mb_level GROUP BY member.mb_level ORDER BY level.ml_idx desc"; 
	...
	/* Location : ./application/models/auth/level_m.php */
?>